# Logical Team Q-learning implementation
...
from catch_prey.LTQL import ltql
...
flags.DEFINE_float('learning_rate', 1e-5, 'learning rate for q network')
flags.DEFINE_float('step_size_ratio', 1.0, 'ratios of c2 to c1 step size')
...
def main(argv):
  ...
  agent = ltql.LTQL(
        ...
        optimizer_opt_q=tf.keras.optimizers.Adam(learning_rate=FLAGS.learning_rate),
  ...
  # Test performance of learnt strategy in situations with different number of agents
  agents_min = 4
  agents_max = 14
  test_win_rate = np.zeros(agents_max - agents_min)
  for agents in range(agents_min, agents_max):
    env = CatchPrey(agent_nmbr=agents)
    test_win_rate[agents - agents_min] = experiment.test(agent=agent, environment=env, num_agents=agents, nmbr_games=10000)
  print(test_win_rate)
  fig, ax = plt.subplots()
  plt.xlabel('Number of agents', fontsize=25)
  plt.ylabel('Test win %', fontsize=25)
  plt.grid()
  ax.scatter(np.arange(start=agents_min, stop=agents_max), test_win_rate * 100)
  plt.ylim(0, 100)
  ax.tick_params(axis='both', which='major', labelsize=15)
  fig.savefig(fname=FLAGS.save_path+'nmbr_agents_test', bbox_inches='tight')
